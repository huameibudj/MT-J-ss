...
def _get_split_image_label_dirs(cfg, split: str):
    data_root = cfg.get("data_root", None)

    # Optional explicit dirs. Useful when split.json stores only filenames/stems.
    image_dir = cfg.get("image_dir", None)
    label_dir = cfg.get("label_dir", cfg.get("mask_dir", None))

    if image_dir is None and data_root is not None:
        image_dir = os.path.join(data_root, split, "images")

    if label_dir is None and data_root is not None:
        label_dir = os.path.join(data_root, split, "masks")
...
def build_dataset_from_cfg(cfg, split: str):
    ...
    if dataset_name in ("glas", "new"):
        from datasets.glas_dataset import GlasDataset, load_split_json_paths

        image_dir, label_dir = _get_split_image_label_dirs(cfg, split)

        if split_file:
            image_paths, label_paths = load_split_json_paths(
                split_file=split_file,
                split=split,
                data_root=data_root,
                image_dir=image_dir,
                label_dir=label_dir,
                mask_suffix=str(cfg.get("mask_suffix", "")),
            )
        ...
        # Backward compatible original folder mode:
        # data_root/{train,val,test}/images and data_root/{train,val,test}/masks
        ds = GlasDataset(
            image_dir=image_dir,
            label_dir=label_dir,
            resize_hw=resize_hw,
        ...
        raise ValueError(f"Unsupported dataset_name: {dataset_name} (use glas/new or glassai)")
...
def get_backbone_module(model):
    """
    兼容不同 EoMT 封装命名，优先定位 ViT backbone。
    """
...
def set_backbone_trainable(model, trainable: bool):
    """
    trainable=False 时：
    1. backbone 内所有参数 requires_grad=False；
    2. backbone.eval()，避免 Dropout / stochastic depth 等训练态随机性；
    3. 后续创建 optimizer 时，这些参数不会进入 optimizer param_groups。
    """
    backbone = get_backbone_module(model)

    if backbone is None:
        print("[WARN] Could not find ViT backbone module to freeze/unfreeze.")
        return 0
...
def assert_backbone_frozen(model, where=""):
    ...
    if not stats["found"]:
        print("[WARN] Could not verify frozen ViT backbone because backbone module was not found.")
        return

    if stats["trainable_params"] != 0:
        suffix = f" at {where}" if where else ""
        raise RuntimeError(
            "[ERROR] ViT backbone should be fully frozen"
...
def main():
    cfg_path = os.environ.get("TRAIN_CONFIG", "configs/trainconfig_new_splitjson.yaml")
    if not os.path.exists(cfg_path):
        cfg_path = "configs/trainconfig_glas.yaml"
    cfg = load_yaml_any_encoding(cfg_path)
    cfg["lr"] = float(cfg["lr"])
    ...
    # --------------------------------------------------------------------------
    # 全冻结消融：冻结 ViT / backbone 预训练权重
    # --------------------------------------------------------------------------
    # 关键点：必须在创建 optimizer 之前冻结。这样 frozen backbone 参数不会进入优化器，
    # 也不会被 AdamW 更新。默认开启；如需做不冻结对照组，可在配置里设：
    ...
    if not param_groups:
        raise RuntimeError(
            "[ERROR] No trainable parameters were found for optimizer. "
            "When ViT/backbone is frozen, decoder/head/query modules must remain trainable."
        )
    ...
    print(
        f"[INFO] Optimizer lr: "
        f"backbone={'frozen/not_in_optimizer' if len(backbone_params) == 0 else base_lr * backbone_lr_mult}, "
        f"joint={base_lr * joint_lr_mult}, "
        f"head={base_lr}"
    )
    print(
        f"[INFO] Optimizer parameter groups: "
        f"backbone={len(backbone_params)}, joint={len(joint_params)}, head={len(head_params)}"
    )

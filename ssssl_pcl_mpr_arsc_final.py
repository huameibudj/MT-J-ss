...
def arsc_ramp_factor(cfg: "CFG", epoch: int) -> float:
    """
    ARSC 延迟启用 + 线性 warmup：
    - epoch < struct_start_epoch：不启用 ARSC，避免 EMA teacher 早期不稳定导致错误结构蒸馏。
    - epoch >= struct_start_epoch：在 struct_warmup_epochs 内逐步升到 1。
    """
    if not cfg.use_struct:
        return 0.0
    if epoch < cfg.struct_start_epoch:
        return 0.0
    if cfg.struct_warmup_epochs <= 0:
        return 1.0
    return min(1.0, max(0.0, (epoch - cfg.struct_start_epoch + 1) / float(cfg.struct_warmup_epochs)))


def active_loss_weights(cfg: "CFG", t_cur: float, struct_factor: float = 1.0):
    """
    PCL + MPR + ARSC 的动态权重。
    注意：ARSC 的权重会乘以 struct_factor，训练前期自动关闭/渐进打开。
    """
    raw = {
        "pcl": ((0.25 + 0.15 * t_cur) * cfg.w_pcl) if cfg.use_pcl else 0.0,
        "mpr": ((0.50 - 0.20 * t_cur) * cfg.w_mpr) if cfg.use_mpr else 0.0,
        "struct": ((0.25 + 0.05 * t_cur) * cfg.w_struct * struct_factor) if cfg.use_struct else 0.0,
    }
    total = float(sum(raw.values()))
    if total <= 0:
        raise ValueError(
            "至少需要开启一个有效训练目标。若只开启 ARSC，请将 struct_start_epoch 设为 1，"
            "或同时开启 use_pcl / use_mpr。"
        )
...
def _region_pool_tokens(patch: torch.Tensor, grid: int, num_regions: int) -> torch.Tensor:
    """把 patch token 池化成规则区域 token，用于多尺度关系结构约束。"""
    B, N, C = patch.shape
    if N != grid * grid:
        raise ValueError(f"patch 数量 N={N} 与 grid={grid} 不匹配，期望 N={grid * grid}")

    rs = max(1, int(round(math.sqrt(num_regions))))
    block = grid // rs

    # 如果 num_regions 不能形成规则划分，退化为全局区域 token，避免训练中断。
    if block <= 0 or block * rs != grid:
        return patch.mean(dim=1, keepdim=True)

    x = patch.view(B, grid, grid, C)
    pooled = []
    for i in range(rs):
        for j in range(rs):
            pooled.append(x[:, i * block:(i + 1) * block, j * block:(j + 1) * block, :].mean(dim=(1, 2)))
    return torch.stack(pooled, dim=1)


def _confidence_guided_relation_kd(
    ...
    """
    Confidence-Guided Relational KD.
    不直接约束 student/teacher 的 token 特征值相同，而是约束 token-token 关系分布一致。
    teacher 关系分布越尖锐，结构置信度越高，对应 token 的结构蒸馏权重越大。
    """
    B, N, _ = s_tokens.shape
    if N <= 1:
        return s_tokens.sum() * 0.0

    tau = max(float(temperature), 1e-6)
    s = F.normalize(s_tokens, dim=-1)
    t = F.normalize(t_tokens.detach(), dim=-1)

    s_logits = torch.matmul(s, s.transpose(1, 2)) / tau
    t_logits = torch.matmul(t, t.transpose(1, 2)) / tau

    # 去掉 self-relation，让结构约束关注 patch/region 之间的关系。
    eye = torch.eye(N, device=s_tokens.device, dtype=torch.bool).unsqueeze(0)
    s_logits = s_logits.masked_fill(eye, -1e4)
    t_logits = t_logits.masked_fill(eye, -1e4)

    with torch.no_grad():
        t_prob = F.softmax(t_logits, dim=-1)
        t_log_prob = torch.log(t_prob.clamp_min(1e-8))
        entropy = -(t_prob * t_log_prob).sum(dim=-1)
        max_entropy = math.log(max(N - 1, 2))
        confidence = (1.0 - entropy / max_entropy).clamp(0.0, 1.0).pow(conf_power)

    s_log_prob = F.log_softmax(s_logits, dim=-1)
    kd_per_token = F.kl_div(s_log_prob, t_prob, reduction="none").sum(dim=-1)
    return (kd_per_token * confidence).sum() / confidence.sum().clamp_min(1.0)


def adaptive_relational_structure_loss(
    ...
    组成：
    1) Relational：约束 patch/region 之间的关系结构，而不是直接做特征 MSE；
    2) Confidence-Guided：根据 teacher 关系分布熵给结构蒸馏加权；
    3) Multi-scale：同时包含 patch-level、fine region-level、coarse region-level 结构。
    """
    pw = max(float(patch_weight), 0.0)
    rw = max(float(region_weight), 0.0)
    norm = max(pw + rw, 1e-6)
    pw, rw = pw / norm, rw / norm

    loss_patch = _confidence_guided_relation_kd(
        s_patch,
        t_patch,
    ...
    fine_regions = max(1, int(num_regions))
    coarse_regions = max(4, fine_regions // 4)

    s_fine = _region_pool_tokens(s_patch, grid, fine_regions)
    t_fine = _region_pool_tokens(t_patch, grid, fine_regions)
    loss_fine = _confidence_guided_relation_kd(
        s_fine,
        t_fine,
        temperature=temperature,
        conf_power=conf_power,
    ...
    s_coarse = _region_pool_tokens(s_patch, grid, coarse_regions)
    t_coarse = _region_pool_tokens(t_patch, grid, coarse_regions)
    loss_coarse = _confidence_guided_relation_kd(
        s_coarse,
        t_coarse,
        temperature=temperature,
        conf_power=conf_power,
    ...
    loss_region = 0.7 * loss_fine + 0.3 * loss_coarse
    return pw * loss_patch + rw * loss_region
...
class CFG:
    ...
    w_pcl: float = 1.0
    w_mpr: float = 1.5
    # ARSC 是辅助结构正则，建议默认不要再使用 5.0 这种强约束。
    w_struct: float = 0.5

    # Final setting: PCL + MPR + ARSC
    use_pcl: bool = True
    use_mpr: bool = True
    ...
    # ARSC: Adaptive Relational Structural Consistency
    struct_start_epoch: int = 10
    struct_warmup_epochs: int = 10
    struct_temperature: float = 0.2
    struct_conf_power: float = 1.5
    arsc_patch_weight: float = 0.6
    arsc_region_weight: float = 0.4
...
def validate_cfg(cfg: CFG):
    ...
    if cfg.use_struct:
        if cfg.struct_start_epoch < 1:
            raise ValueError("struct_start_epoch 必须 >= 1")
        if cfg.struct_warmup_epochs < 0:
            raise ValueError("struct_warmup_epochs 必须 >= 0")
        if cfg.struct_temperature <= 0:
            raise ValueError("struct_temperature 必须大于 0")
        if cfg.arsc_patch_weight < 0 or cfg.arsc_region_weight < 0:
            raise ValueError("arsc_patch_weight / arsc_region_weight 不能为负数")
        if cfg.arsc_patch_weight + cfg.arsc_region_weight <= 0:
            raise ValueError("arsc_patch_weight 与 arsc_region_weight 不能同时为 0")
...
def main(cfg_path: Optional[str] = None):
    ...
    if cfg_path is None:
        cfg_path = os.path.join(script_dir, "configs", "ssssl_pcl_mpr_arsc_final.yaml")
    else:
    ...
    print(
        "[INFO] 训练目标: "
        f"PCL={cfg.use_pcl}, MPR={cfg.use_mpr}, ARSC={cfg.use_struct}"
    )
    print(
        f"[INFO] ARSC: start_epoch={cfg.struct_start_epoch}, "
        f"warmup_epochs={cfg.struct_warmup_epochs}, w_struct={cfg.w_struct}, "
        f"tau={cfg.struct_temperature}, conf_power={cfg.struct_conf_power}"
    )
    if cfg.use_struct and cfg.w_struct > 1.0:
        print("[WARN] 当前 w_struct > 1.0。ARSC 建议先使用 0.3~1.0，过强结构约束可能压制病理局部异质性。")
    ...
    try:
        for epoch in range(start_epoch, cfg.epochs + 1):
            ...
            struct_factor = arsc_ramp_factor(cfg, epoch)
            use_arsc_now = bool(cfg.use_struct and struct_factor > 0.0)
            weights = active_loss_weights(cfg, t_cur, struct_factor=struct_factor)

            pbar = tqdm.tqdm(dl, desc=f"Epoch {epoch}/{cfg.epochs}", ncols=155)

            for _, (xw, xs) in enumerate(pbar, start=1):
                xw = xw.to(device, non_blocking=True)
                xs = xs.to(device, non_blocking=True)

                need_student_clean = cfg.use_pcl or use_arsc_now
                ...
                with torch.cuda.amp.autocast(enabled=amp_enabled):
                    ...
                    if use_arsc_now:
                        l_struct = adaptive_relational_structure_loss(
                            patch_s,
                            patch_t,
                            grid,
                            cfg.num_regions,
                            temperature=cfg.struct_temperature,
                            conf_power=cfg.struct_conf_power,
                            patch_weight=cfg.arsc_patch_weight,
                            region_weight=cfg.arsc_region_weight,
                        )
                ...
                pbar.set_postfix({
                    "loss": f"{loss.item():.4f}",
                    "pcl": f"{l_pcl.item():.4f}" if cfg.use_pcl else "off",
                    "mpr": f"{l_mpr.item():.4f}" if cfg.use_mpr else "off",
                    "arsc": f"{l_struct.item():.4f}" if use_arsc_now else ("delay" if cfg.use_struct else "off"),
                    "arsc_w": f"{weights['struct']:.3f}",
                    "lr_bb": f"{optim.param_groups[0]['lr']:.2e}",
                    "lr_hd": f"{optim.param_groups[1]['lr']:.2e}",
                })
...
def build_argparser():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="configs/ssssl_pcl_mpr_arsc_final.yaml",
    )
    return parser
